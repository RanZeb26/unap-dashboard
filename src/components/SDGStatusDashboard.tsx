        ...
        <div className="flex items-center gap-2 bg-slate-100 p-1.5 rounded-full border border-slate-200">
          ...
          {["All", "Poverty", "Health", "Climate"].map((filter) => (
            <button
              ...
              className="text-xs px-3.5 py-1.5 rounded-full font-bold transition-all bg-white text-slate-800 hover:bg-unap-brand hover:text-slate-950 shadow-sm"
            >
              {filter}
            </button>
          ))}
          ...
          return (
            <button
              ...
              className={`p-3 rounded-2xl text-left transition-all duration-200 border flex flex-col justify-between h-24 ${
                isSelected
                  ? "bg-white border-unap-brand shadow-lg ring-2 ring-unap-brand/20 scale-[1.02]"
        ...
        <div className="bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 shadow-sm flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <span className="text-[10px] font-bold text-unap-brand uppercase tracking-wider block">
                  Active Focus Target
                </span>
            ...
            <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 space-y-2 mb-6">
              ...
              <div className="flex justify-between text-xs">
                ...
                <span className="font-black text-unap-brand text-sm">{selectedGoal.keyMetricValue}</span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2 pt-4 border-t border-slate-200">
            <div className="flex justify-between items-center text-xs">
              ...
              <span className="text-unap-brand font-black text-base">{selectedGoal.globalAverage}%</span>
            </div>
          ...
          <div className="flex justify-between items-center mb-6">
            <div>
              <h3 className="text-lg font-bold text-slate-900 flex items-center gap-2">
                <Globe className="w-5 h-5 text-unap-brand" />
                Regional Index Breakdown
              </h3>
              <p className="text-xs text-slate-500">Statista & Kaggle multi-continent comparative data</p>
            </div>
            <span className="text-xs font-bold px-3 py-1 rounded-full bg-sky-50 text-unap-brand border border-sky-100 flex items-center gap-1">
